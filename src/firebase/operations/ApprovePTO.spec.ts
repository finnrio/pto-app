...
describe("ApprovePTO", () => {
  ...
  describe("valid pto request", () => {
    ...
    it("should update the pto status to approved", async () => {
      expect((await getDoc(doc(testUserDocRef, "pto", testPTOId))).data()).toEqual(expect.objectContaining({ status: "Approved" }));
    });
    ...
    it("should add the hours from the users pto_used", async () => {
      expect((await getDoc(testUserDocRef)).data()).toEqual(expect.objectContaining({ pto_used: 8 }));
    });
  });
  describe("invalid pto request", () => {
    it("should error", async () => {
      await expect(ApprovePTO(testUserId, "invalidPtoId")).rejects.toThrow("PTO does not exist");
    });
  });
