...
const testSubordinate1DocRef = doc(FIRESTORE_DB, `${NODE_ENV}`, testSubordinateId1);
const testSubordinate2DocRef = doc(FIRESTORE_DB, `${NODE_ENV}`, testSubordinateId2);
...
describe("GetSubordinates", () => {
  ...
  it("should return an array of subordinates", async () => {
    expect(await GetSubordinates(testUserId)).toEqual([{ id: testSubordinateId1 }, { id: testSubordinateId2 }]);
  });
