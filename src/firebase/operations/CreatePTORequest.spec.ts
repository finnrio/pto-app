...
describe("CreatePTORequest", () => {
  describe("when successful", () => {
    ...
    beforeAll(async () => {
      ...
      ptoUuid = await CreatePTORequest(
        testUserId,
        new Date("2024-01-01"),
        new Date("2024-02-01"),
        "purpose",
    ...
    it("should generate a pto request", async () => {
      expect(
        (await getDoc(doc(testUserDocRef, "pto", ptoUuid))).exists(),
      ).toBeTruthy();
    });
  ...
  describe("with an invalid user", () => {
    it("throws expected error", async () => {
      ...
      ).rejects.toThrow("No user data found");
    });
  });
  describe("insufficient allowance", () => {
    ...
    it("throws expected error", async () => {
      ...
      ).rejects.toThrow("Not enough PTO hours available");
    });
  });
